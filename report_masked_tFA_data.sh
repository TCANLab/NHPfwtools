...
#uncomment to loop through or select an age
#foreach age ( '6M' '12M' '24M' '36M')
foreach age ( '45M' )

#uncomment to choose white matter or gray matter sufx
#foreach sufx ( 'UNCParc_2' 'UNCParc_1' 'cleanWM')
foreach sufx ( 'UNCParc_MultiAtlas_1' 'UNCParc_MultiAtlas_2' 'cleanWMGM_nosubcort')
# ------------------------------------------------------------------
#find all subjects
#can uncomment to run all subjects or specific ones
...
foreach i ( $all )
	 ...
	 set onam = tFAmap_${i}_${sufx}.rez;
	 #echo $matlas;
	 set tFAmap = "${Res}/${i}/dtt2ants/tFA2t2_nl.nii.gz";
	 ...
	 if ( ${sufx} == "cleanWMGM_nosubcort" ) then
		  ...
		  foreach k ( 1 2 )
				...
				echo "${j}, ${age}, ${k}, ${sufx}, `fslstats ${Res}/${i}/dtt2ants/tFA2t2_nl.nii.gz -k msk_${k}_${sufx} -M`" >> $onam;
		  end
	 else
		  set k = "";
		  ...
		  echo "${j}, ${age}, prefront, ${sufx}, `fslstats ${Res}/${i}/dtt2ants/tFA2t2_nl.nii.gz -k msk_prefront_${sufx} -M`" >> $onam;
		  [ -f msk_midfront_${sufx}.nii.gz ]|| fslmaths msk_4_${sufx} -add msk_17_${sufx} -bin msk_midfront_${sufx};
		  echo "${j}, ${age}, midfront, ${sufx}, `fslstats ${Res}/${i}/dtt2ants/tFA2t2_nl.nii.gz -k msk_midfront_${sufx} -M`" >> $onam;
		  [ -f msk_cingulate_${sufx}.nii.gz ]|| fslmaths msk_7_${sufx} -add msk_20_${sufx} -bin msk_cingulate_${sufx};
		  echo "${j}, ${age}, cingulate, ${sufx}, `fslstats ${Res}/${i}/dtt2ants/tFA2t2_nl.nii.gz -k msk_cingulate_${sufx} -M`" >> $onam;
		  [ -f msk_medtemp_${sufx}.nii.gz ]|| fslmaths msk_12_${sufx} -add msk_25_${sufx} -bin msk_medtemp_${sufx};
		  echo "${j}, ${age}, medtemp, ${sufx}, `fslstats ${Res}/${i}/dtt2ants/tFA2t2_nl.nii.gz -k msk_medtemp_${sufx} -M`" >> $onam;
	 endif
